...
def train(hyperparameters, num_gpus, current_host, output_data_dir, hosts):
    ...
    print('process {} on host {} of {} starting'.format(
        comm.intra_rank, current_host, len(hosts)))
    ...
    if comm.intra_rank == 1 and current_host != 'algo-1':
        ...
        time.sleep(6)
        open(os.path.join(output_data_dir, 'process_could_complete'),
             'a').close()

    print('process {} on host {} exiting'.format(comm.intra_rank,
                                                 current_host))
