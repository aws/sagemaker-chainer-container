...
resource_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', '..', 'resources', 'failure_scenarios')
...
def test_all_processes_finish_with_mpi(docker_image, opt_ml, use_gpu):
    ...
    assert local_mode.file_exists(opt_ml, 'output/data/process_could_complete', host='algo-2'), \
        'Model was not saved'
