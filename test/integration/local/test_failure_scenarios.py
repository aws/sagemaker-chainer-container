...
def test_all_processes_finish_with_mpi(docker_image, opt_ml, use_gpu):
    ...
    cluster_size = 2
    hyperparameters = {'use_mpi': True, 'process_slots_per_host': 2, 'num_processes': 4}
...
def test_training_jobs_do_not_stall(docker_image, opt_ml, use_gpu):
    ...
    cluster_size = 2
    hyperparameters = {'use_mpi': True, 'process_slots_per_host': 1, 'num_processes': 2}
...
def test_distributed_failure(docker_image, opt_ml, use_gpu):
    customer_script = 'failure_script.py'
    cluster_size = 2
    hyperparameters = {'rank': 'inter_rank', 'process_slots_per_host': 1,
                       'num_processes': cluster_size, 'node_to_fail': 1}
